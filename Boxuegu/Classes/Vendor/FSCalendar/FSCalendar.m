...
- (BOOL)isDateInRange:(NSDate *)date
{
    ...
    if(!flag)
    {
        //[self.delegateProxy calendarLessThanMinimumDate:self];
    }
    flag &= [self.gregorian components:NSCalendarUnitDay fromDate:date toDate:self.maximumDate options:0].day >= 0;
    if(!flag)
    {
        //[self.delegateProxy calendarLargerThanMaximumDate:self];
    }
    return flag;
}
    ...
    if (!self.floatingMode) {
        ...
        if (_scrollEnabled) {
            if (!_deliver) {
                ...
                deliver.userInteractionEnabled = NO;
                deliver.backgroundColor = [UIColor clearColor];
